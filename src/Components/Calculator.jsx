...
export default function Calculator() {
  ...
  async function fetchData() {
    try {
      showNotification('Processing your files...! This may take a few minutes', 'info')
      const allScannedItems = await Promise.all(files.map(async (file) => {
      ...
      const allNewItems = allScannedItems.flatMap(result => result.items);

      setItems(prev => [...prev, ...allNewItems]);
      setFiles([])

      hideNotification();
      showNotification(`Successfully processed ${allScannedItems.length} receipt(s)!`, 'success');
    } catch (error) {
      hideNotification();
      showNotification('Error Processing Receipts! Please try again later.', 'error');
